...
An individual modification of the newly added field *tx_examples_main_category* to the TCA definition of the table *tt_content* can be done in the TCE (TYPO3 Core Engine) TSConfig. In most cases you want to set the page id of the general storage folder (available as a plugin select box until TYPO3 6.2). Tnen the examples extension will only use the content records from the given page id. ::
